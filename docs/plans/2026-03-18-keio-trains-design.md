...
- `data/train-seeds.json` を正本にして、京王の `7000 / 8000 / 9000 / 2000 / 高尾山口行き` を 5 件の独立エントリとして追加する
...
- `高尾山口行き` は系式ではなく行先差分だが、Issue 本文に明示されているため独立問題として保持した方が期待に沿う
...
- `高尾山口行き` は通常塗装との差分が見えやすい写真が取れない場合、Issue 本文の行先指定を根拠にしつつ、高尾線で走る公開画像を優先する
