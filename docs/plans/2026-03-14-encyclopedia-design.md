...
- `data/train-seeds.json` と `data/trains.json` に、親向け補足を入れる `encyclopedia` オブジェクトを全車両へ持たせる
...
- `descriptionShort` は公開情報で確認できた事実をもとに、1文で「特徴」と「走る場所/役割」を伝える
- 似た車両や派生車は、並べて読んだときに違いが分かるよう `見た目 / 会社 / 役割 / 走行区間` のどれかで必ず書き分ける
- `data/train-seeds.json` を説明文の正本とし、`data/trains.json` には同じ文面を反映する
- 親向け補足は `encyclopedia.routeSummary / featureSummary / speedLabel / topSpeedKmh` を全車両で必須にし、比較したい車両だけ `comparison` を持たせる
