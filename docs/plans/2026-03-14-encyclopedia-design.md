...
- `data/train-seeds.json` と `data/trains.json` に、親向け補足を入れる `encyclopedia` オブジェクトを段階導入する
...
- 親向け補足は `encyclopedia.routeSummary / featureSummary / speedLabel / topSpeedKmh` を基本必須にし、比較したい車両だけ `comparison` を持たせる
