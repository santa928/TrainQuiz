...
2026年9月12日の[呼び名改善と全110件の名称点検](train-names.md)で500系・N700系・N700A・0系・E4系・E3系2000番台・E231系近郊形・E233系湘南色の表示名を更新した。本監査とJSON台帳の名前・画面寸法は9月8日時点の記録として保持し、新しい名前の画面確認はリンク先に記録する。写真の変更はない。
...
| REQ-008 | 追加 | レビューで残件として指摘された500系通常色を「500系新幹線（通常色）」と明示し、ハローキティ新幹線との同時4択を両方向で検証する |

要件差分はREQ-001〜006を維持、REQ-007を追加。保留・削除はない。通常色・特別塗装を同時に含む4択を検証用に生成し、選択肢名の意味が重ならないことを確認した。毎回同形式ペアを出題する仕様への変更は行わない。

PRレビュー対応ではREQ-001〜007を維持し、REQ-008を追加した。初回監査では500系の一般名称とラッピング名の意味の重なりを取り残していた。[JR西日本公式](https://www.westjr.co.jp/press/article/2018/06/page_12571.html)でハローキティ新幹線が500系であることを確認し、通常色の表示名・seed・公開JSON・監査台帳を同期した。390×844で両方を含む4択を両方向で再撮影し、文言とボタン内の収まり・横幅390pxを確認した。出典や写真の変更はなく、source lockは維持した。
...
- Nodeテスト66件が成功。500系とハローキティを含む同時4択テストは、変更前の一般名称で失敗することを確認した。写真・生成処理が変わらないPythonテスト10件と全107写真の検証は初回PRの成功証拠を再利用する。写真差し替えの回帰テストも変更前の不一致で失敗することを確認済み。
