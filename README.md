...
- 正解するまで選び直せる子ども向け導線
- 色で案内しやすい4色の選択肢ボタン
- 読み上げモード：4つの選択肢を色と名前で順番に読み、🔊で個別に聞き直せる
...
- 字を読む前のお子さんには、タイトルで「🔇 よみあげ OFF」を押してONにしてから始めてください。問題ごとに4つの名前をゆっくり読み、読んでいる選択肢を青い枠で示します。
- 各選択肢の「🔊」は聞き直し専用です。回答するには色のついた名前のボタンを押します。「🔊 ぜんぶ きく」で最初から聞き直せます。設定は同じブラウザに保存します。
- 読み上げは端末内の日本語音声を使います。iPhone / iPadはSafariで開き、端末の音量を上げてお試しください。日本語音声がない場合や再生に失敗した場合は画面に案内を出します。詳しくは[読み上げモード](docs/read-aloud.md)を参照してください。
...
写真は号機・特別塗装・改良型まで表示名と一致させ、対象の1編成が主役になるものを選びます。390×844で色・先頭形状・ラッピングを確認し、クイズ・図鑑とも写真全体を表示します。[Issue #13の全107問監査と15写真の修正記録](docs/photo-audit-2026-09.md)に出典と実測結果をまとめています。
